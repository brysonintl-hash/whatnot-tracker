...
function BookmarkletSection({ appUrl }: { appUrl: string }) {
  // no-cors bypasses all CORS restrictions — request goes through without browser blocking it
  const bm = `javascript:(function(){var m=location.href.match(/whatnot\\.com\\/user\\/([^/?#]+)/);if(!m){alert('Go to a Whatnot seller shop page first (e.g. whatnot.com/user/toolsforlifee/shop)');return;}var username=m[1].toLowerCase();var seen=new Set();var listings=[];document.querySelectorAll('a[href*="/listing/"]').forEach(function(a){var hm=(a.href||'').match(/\\/listing\\/([a-zA-Z0-9_=+\\-]{6,})/);if(!hm||seen.has(hm[1]))return;seen.add(hm[1]);var id=hm[1];var card=a;for(var i=0;i<8;i++){if(card.parentElement)card=card.parentElement;else break;}var titleEl=card.querySelector('[data-testid*="title"],[data-testid*="name"]');var title=titleEl?titleEl.textContent.trim():'';if(!title){var img=a.querySelector('img');title=img?img.alt.trim():'';}if(!title||title.length<4)return;var priceM=(card.textContent||'').match(/\\$(\\d+(?:\\.\\d{1,2})?)/);var price=priceM?parseFloat(priceM[1]):null;var qtyM=(card.textContent||'').match(/Qty[.:\\s]+(\\d+)|(\\d+)\\s+Available/i);var qty=qtyM?parseInt(qtyM[1]||qtyM[2]):null;listings.push({id:id,title:title,price:price,qty:qty,url:'https://www.whatnot.com/listing/'+id});});if(!listings.length){alert('No listings found. Make sure you are on the Shop tab and scroll down first.');return;}var APP='${appUrl}';fetch(APP+'/api/scraper/ingest',{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({username:username,listings:listings}),mode:'no-cors'}).then(function(){setTimeout(function(){window.open(APP+'/scraper?u='+encodeURIComponent(username),'_blank');},400);}).catch(function(){window.open(APP+'/scraper?u='+encodeURIComponent(username),'_blank');});})();`;
