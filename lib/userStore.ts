...
function seed() {
  ...
  const defaults: StoredUser[] = [
    { id: '1', username: process.env.ADMIN_USERNAME || 'admin', password: process.env.ADMIN_PASSWORD || 'admin123', name: 'Administrator', role: 'admin', createdAt: new Date().toISOString() },
    { id: '2', username: process.env.MANAGER_USERNAME || 'manager', password: process.env.MANAGER_PASSWORD || 'manager123', name: 'Manager', role: 'manager', createdAt: new Date().toISOString() },
    { id: '3', username: process.env.SHIPPER_USERNAME || 'shipper', password: process.env.SHIPPER_PASSWORD || 'shipper123', name: 'Shipper', role: 'shipper', createdAt: new Date().toISOString() },
    { id: '4', username: process.env.HOST_USERNAME || 'host', password: process.env.HOST_PASSWORD || 'host123', name: 'Host', role: 'host', createdAt: new Date().toISOString() },
  ];
...
export function createUser(data: { username: string; password: string; name: string; role: Role }): StoredUser {
  seed();
  const user: StoredUser = { ...data, id: Date.now().toString(), createdAt: new Date().toISOString() };
