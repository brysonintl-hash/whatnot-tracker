...
const PRIORITIES = [
  { label: 'Nice to have', sel: 'bg-slate-600 border-slate-600 text-white', unsel: 'border-slate-700 text-slate-400 bg-slate-800' },
  { label: 'Important',    sel: 'bg-amber-500 border-amber-500 text-white', unsel: 'border-slate-700 text-slate-400 bg-slate-800' },
  { label: 'Critical',     sel: 'bg-red-500 border-red-500 text-white',     unsel: 'border-slate-700 text-red-400 bg-slate-800' },
] as const;
...
export default function FeatureRequestModal({ userName, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div className="relative bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg border border-slate-800 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs mt-0.5">Help us build a better app</p>
              </div>
            </div>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 text-sm bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 text-sm bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none"
              />
            </div>
            ...
            {/* Error */}
            {errMsg && (
              <p className="text-xs text-red-400 bg-red-900/20 border border-red-800 rounded-lg px-3 py-2">{errMsg}</p>
            )}

            {/* Actions */}
            <div className="flex gap-3 pt-1 pb-2">
              <button onClick={onClose}
                className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-slate-700 text-slate-400 hover:bg-slate-800 transition-colors"
              >Cancel</button>
